...
class SHARIFSyntaxError(Exception):
    pass

# Hardcoded configuration
LANGUAGE_NAME = "SHARIF"
FILE_EXTENSION = ".sharif"
COMMAND_PREFIX = "SHARIF"
COMMENT_PREFIX = "SHARIFSAY"
ENFORCE_UPPERCASE_COMMENTS = True
ENFORCE_UPPERCASE_CODE = True
TYPE_PREFIX_INTEGER = "SHARIFINT"
TYPE_PREFIX_FLOAT = "SHARIFFLOAT"
TYPE_PREFIX_STRING = "SHARIFSTRING"
TRUE_VALUE = "JUICYBAYBEE"
FALSE_VALUE = "OTHERFRIENDS"
BLOCK_END = "SHARIFEND"
    ...
    for line_number, line in enumerate(lines, 1):
        if COMMENT_PREFIX in line:
            ...
            if ENFORCE_UPPERCASE_COMMENTS and any(c.islower() for c in line[comment_start:]):
                raise SHARIFSyntaxError(f"Syntax Error on line {line_number}: Comments must be in all caps")
...
def parse(tokens):
    ...
    def parse_factor():
        if not tokens:
            raise SHARIFSyntaxError("Unexpected end of input")
        if tokens[0] == '(':
            tokens.pop(0)  # Remove '('
            result = parse_expression()
            if not tokens or tokens.pop(0) != ')':
                raise SHARIFSyntaxError("Expected closing parenthesis")
            return result
            ...
            try:
                ...
                raise SHARIFSyntaxError(f"Unexpected token: {tokens[0]}")
        ...
        if tokens and tokens[0] == '(':
            ...
            while tokens and tokens[0] != ')':
                ...
                if tokens and tokens[0] == ',':
                    tokens.pop(0)
            if not tokens or tokens.pop(0) != ')':
                raise SHARIFSyntaxError(f"Expected closing parenthesis in function call to {name}")
    ...
    def parse_command():
        ...
        if token in [TYPE_PREFIX_INTEGER, TYPE_PREFIX_FLOAT, TYPE_PREFIX_STRING]:
            if not tokens:
                raise SHARIFSyntaxError(f"Expected value after {token}")
            value = tokens.pop(0)
            ...
            raise SHARIFSyntaxError(f"Unknown command: {token}")
    ...
    def parse_if_statement():
        ...
        if not tokens or tokens.pop(0) != ':':
            raise SHARIFSyntaxError("Expected ':' after if condition")
        if_body = []
        ...
        if tokens and tokens[0] == f"{COMMAND_PREFIX}ELSE":
            ...
            if not tokens or tokens.pop(0) != ':':
                raise SHARIFSyntaxError("Expected ':' after else")
            else_body = []
            while tokens and tokens[0] != BLOCK_END:
                else_body.append(parse_statement())
        if not tokens or tokens.pop(0) != BLOCK_END:
            raise SHARIFSyntaxError(f"Expected '{BLOCK_END}' at the end of if statement")
    ...
    def parse_while_loop():
        ...
        condition = parse_expression()
        if not tokens or tokens.pop(0) != ':':
            raise SHARIFSyntaxError("Expected ':' after while condition")
        body = []
        while tokens and tokens[0] != BLOCK_END:
            body.append(parse_statement())
        if not tokens or tokens.pop(0) != BLOCK_END:
            raise SHARIFSyntaxError(f"Expected '{BLOCK_END}' at the end of while loop")
