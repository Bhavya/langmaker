...
class MatthewSyntaxError(Exception):
    pass

# Hardcoded configuration
LANGUAGE_NAME = "Matthew"
FILE_EXTENSION = ".bntz"
COMMAND_PREFIX = "Matthew"
COMMENT_PREFIX = "MatthewComment"
ENFORCE_UPPERCASE_COMMENTS = False
ENFORCE_UPPERCASE_CODE = False
TYPE_PREFIX_INTEGER = "MatthewNum"
TYPE_PREFIX_FLOAT = "MatthewFloat"
TYPE_PREFIX_STRING = "MatthewString"
TRUE_VALUE = "facts"
FALSE_VALUE = "cap"
BLOCK_END = "MatthewEnd"
    ...
    for line_number, line in enumerate(lines, 1):
        if COMMENT_PREFIX in line:
            ...
            if ENFORCE_UPPERCASE_COMMENTS and any(c.islower() for c in line[comment_start:]):
                raise MatthewSyntaxError(f"Syntax Error on line {line_number}: Comments must be in all caps")
...
def parse(tokens):
    ...
    def parse_factor():
        if not tokens:
            raise MatthewSyntaxError("Unexpected end of input")
        if tokens[0] == '(':
            tokens.pop(0)  # Remove '('
            result = parse_expression()
            if not tokens or tokens.pop(0) != ')':
                raise MatthewSyntaxError("Expected closing parenthesis")
            return result
            ...
            try:
                ...
                raise MatthewSyntaxError(f"Unexpected token: {tokens[0]}")
        ...
        if tokens and tokens[0] == '(':
            ...
            while tokens and tokens[0] != ')':
                ...
                if tokens and tokens[0] == ',':
                    tokens.pop(0)
            if not tokens or tokens.pop(0) != ')':
                raise MatthewSyntaxError(f"Expected closing parenthesis in function call to {name}")
    ...
    def parse_command():
        ...
        if token in [TYPE_PREFIX_INTEGER, TYPE_PREFIX_FLOAT, TYPE_PREFIX_STRING]:
            if not tokens:
                raise MatthewSyntaxError(f"Expected value after {token}")
            value = tokens.pop(0)
            ...
            raise MatthewSyntaxError(f"Unknown command: {token}")
    ...
    def parse_if_statement():
        ...
        if not tokens or tokens.pop(0) != ':':
            raise MatthewSyntaxError("Expected ':' after if condition")
        if_body = []
        ...
        if tokens and tokens[0] == f"{COMMAND_PREFIX}ELSE":
            ...
            if not tokens or tokens.pop(0) != ':':
                raise MatthewSyntaxError("Expected ':' after else")
            else_body = []
            while tokens and tokens[0] != BLOCK_END:
                else_body.append(parse_statement())
        if not tokens or tokens.pop(0) != BLOCK_END:
            raise MatthewSyntaxError(f"Expected '{BLOCK_END}' at the end of if statement")
    ...
    def parse_while_loop():
        ...
        condition = parse_expression()
        if not tokens or tokens.pop(0) != ':':
            raise MatthewSyntaxError("Expected ':' after while condition")
        body = []
        while tokens and tokens[0] != BLOCK_END:
            body.append(parse_statement())
        if not tokens or tokens.pop(0) != BLOCK_END:
            raise MatthewSyntaxError(f"Expected '{BLOCK_END}' at the end of while loop")
...
def run(code):
    ...
    except MatthewSyntaxError as e:
