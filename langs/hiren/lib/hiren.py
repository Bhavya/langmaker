...
class HIRENSyntaxError(Exception):
    pass

# Hardcoded configuration
LANGUAGE_NAME = "HIREN"
FILE_EXTENSION = ".bigh"
COMMAND_PREFIX = "HIREN"
COMMENT_PREFIX = "HIRENSAY"
ENFORCE_UPPERCASE_COMMENTS = True
ENFORCE_UPPERCASE_CODE = True
TYPE_PREFIX_INTEGER = "HIRENUM"
TYPE_PREFIX_FLOAT = "HIRENFLOAT"
TYPE_PREFIX_STRING = "HIRENSTRING"
TRUE_VALUE = "YES"
FALSE_VALUE = "NO"
BLOCK_END = "HIREND"
    ...
    for line_number, line in enumerate(lines, 1):
        if COMMENT_PREFIX in line:
            ...
            if ENFORCE_UPPERCASE_COMMENTS and any(c.islower() for c in line[comment_start:]):
                raise HIRENSyntaxError(f"Syntax Error on line {line_number}: Comments must be in all caps")
...
def parse(tokens):
    ...
    def parse_factor():
        if not tokens:
            raise HIRENSyntaxError("Unexpected end of input")
        if tokens[0] == '(':
            tokens.pop(0)  # Remove '('
            result = parse_expression()
            if not tokens or tokens.pop(0) != ')':
                raise HIRENSyntaxError("Expected closing parenthesis")
            return result
            ...
            try:
                ...
                raise HIRENSyntaxError(f"Unexpected token: {tokens[0]}")
        ...
        if tokens and tokens[0] == '(':
            ...
            while tokens and tokens[0] != ')':
                ...
                if tokens and tokens[0] == ',':
                    tokens.pop(0)
            if not tokens or tokens.pop(0) != ')':
                raise HIRENSyntaxError(f"Expected closing parenthesis in function call to {name}")
    ...
    def parse_command():
        ...
        if token in [TYPE_PREFIX_INTEGER, TYPE_PREFIX_FLOAT, TYPE_PREFIX_STRING]:
            if not tokens:
                raise HIRENSyntaxError(f"Expected value after {token}")
            value = tokens.pop(0)
            ...
            raise HIRENSyntaxError(f"Unknown command: {token}")
    ...
    def parse_if_statement():
        ...
        if not tokens or tokens.pop(0) != ':':
            raise HIRENSyntaxError("Expected ':' after if condition")
        if_body = []
        ...
        if tokens and tokens[0] == f"{COMMAND_PREFIX}ELSE":
            ...
            if not tokens or tokens.pop(0) != ':':
                raise HIRENSyntaxError("Expected ':' after else")
            else_body = []
            while tokens and tokens[0] != BLOCK_END:
                else_body.append(parse_statement())
        if not tokens or tokens.pop(0) != BLOCK_END:
            raise HIRENSyntaxError(f"Expected '{BLOCK_END}' at the end of if statement")
    ...
    def parse_while_loop():
        ...
        condition = parse_expression()
        if not tokens or tokens.pop(0) != ':':
            raise HIRENSyntaxError("Expected ':' after while condition")
        body = []
        while tokens and tokens[0] != BLOCK_END:
            body.append(parse_statement())
        if not tokens or tokens.pop(0) != BLOCK_END:
            raise HIRENSyntaxError(f"Expected '{BLOCK_END}' at the end of while loop")
