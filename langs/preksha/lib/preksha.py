...
class PrekshaSyntaxError(Exception):
    pass

# Hardcoded configuration
LANGUAGE_NAME = "Preksha"
FILE_EXTENSION = ".pk"
COMMAND_PREFIX = "Popo"
COMMENT_PREFIX = "PopoSay"
ENFORCE_UPPERCASE_COMMENTS = False
ENFORCE_UPPERCASE_CODE = False
TYPE_PREFIX_INTEGER = "PopoInt"
TYPE_PREFIX_FLOAT = "PopoFloat"
TYPE_PREFIX_STRING = "PopoString"
TRUE_VALUE = "yeys"
FALSE_VALUE = "naow"
BLOCK_END = "PopoDone"
    ...
    for line_number, line in enumerate(lines, 1):
        if COMMENT_PREFIX in line:
            ...
            if ENFORCE_UPPERCASE_COMMENTS and any(c.islower() for c in line[comment_start:]):
                raise PrekshaSyntaxError(f"Syntax Error on line {line_number}: Comments must be in all caps")
...
def parse(tokens):
    ...
    def parse_factor():
        if not tokens:
            raise PrekshaSyntaxError("Unexpected end of input")
        if tokens[0] == '(':
            tokens.pop(0)  # Remove '('
            result = parse_expression()
            if not tokens or tokens.pop(0) != ')':
                raise PrekshaSyntaxError("Expected closing parenthesis")
            return result
            ...
            try:
                ...
                raise PrekshaSyntaxError(f"Unexpected token: {tokens[0]}")
        ...
        if tokens and tokens[0] == '(':
            ...
            while tokens and tokens[0] != ')':
                ...
                if tokens and tokens[0] == ',':
                    tokens.pop(0)
            if not tokens or tokens.pop(0) != ')':
                raise PrekshaSyntaxError(f"Expected closing parenthesis in function call to {name}")
    ...
    def parse_command():
        ...
        if token in [TYPE_PREFIX_INTEGER, TYPE_PREFIX_FLOAT, TYPE_PREFIX_STRING]:
            if not tokens:
                raise PrekshaSyntaxError(f"Expected value after {token}")
            value = tokens.pop(0)
            ...
            raise PrekshaSyntaxError(f"Unknown command: {token}")
    ...
    def parse_if_statement():
        ...
        if not tokens or tokens.pop(0) != ':':
            raise PrekshaSyntaxError("Expected ':' after if condition")
        if_body = []
        ...
        if tokens and tokens[0] == f"{COMMAND_PREFIX}ELSE":
            ...
            if not tokens or tokens.pop(0) != ':':
                raise PrekshaSyntaxError("Expected ':' after else")
            else_body = []
            while tokens and tokens[0] != BLOCK_END:
                else_body.append(parse_statement())
        if not tokens or tokens.pop(0) != BLOCK_END:
            raise PrekshaSyntaxError(f"Expected '{BLOCK_END}' at the end of if statement")
    ...
    def parse_while_loop():
        ...
        condition = parse_expression()
        if not tokens or tokens.pop(0) != ':':
            raise PrekshaSyntaxError("Expected ':' after while condition")
        body = []
        while tokens and tokens[0] != BLOCK_END:
            body.append(parse_statement())
        if not tokens or tokens.pop(0) != BLOCK_END:
            raise PrekshaSyntaxError(f"Expected '{BLOCK_END}' at the end of while loop")
...
def run(code):
    ...
    except PrekshaSyntaxError as e:
