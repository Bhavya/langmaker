...
class MATTSyntaxError(Exception):
    pass

# Hardcoded configuration
LANGUAGE_NAME = "MATT"
FILE_EXTENSION = ".selby"
COMMAND_PREFIX = "MATT"
COMMENT_PREFIX = "MATTEXCLAIM"
ENFORCE_UPPERCASE_COMMENTS = True
ENFORCE_UPPERCASE_CODE = True
TYPE_PREFIX_INTEGER = "MATTINT"
TYPE_PREFIX_FLOAT = "MATTFLOAT"
TYPE_PREFIX_STRING = "MATTSTRING"
TRUE_VALUE = "YES"
FALSE_VALUE = "NO"
BLOCK_END = "MATTEND"
    ...
    for line_number, line in enumerate(lines, 1):
        if COMMENT_PREFIX in line:
            ...
            if ENFORCE_UPPERCASE_COMMENTS and any(c.islower() for c in line[comment_start:]):
                raise MATTSyntaxError(f"Syntax Error on line {line_number}: Comments must be in all caps")
...
def parse(tokens):
    ...
    def parse_factor():
        if not tokens:
            raise MATTSyntaxError("Unexpected end of input")
        if tokens[0] == '(':
            tokens.pop(0)  # Remove '('
            result = parse_expression()
            if not tokens or tokens.pop(0) != ')':
                raise MATTSyntaxError("Expected closing parenthesis")
            return result
            ...
            try:
                ...
                raise MATTSyntaxError(f"Unexpected token: {tokens[0]}")
        ...
        if tokens and tokens[0] == '(':
            ...
            while tokens and tokens[0] != ')':
                ...
                if tokens and tokens[0] == ',':
                    tokens.pop(0)
            if not tokens or tokens.pop(0) != ')':
                raise MATTSyntaxError(f"Expected closing parenthesis in function call to {name}")
    ...
    def parse_command():
        ...
        if token in [TYPE_PREFIX_INTEGER, TYPE_PREFIX_FLOAT, TYPE_PREFIX_STRING]:
            if not tokens:
                raise MATTSyntaxError(f"Expected value after {token}")
            value = tokens.pop(0)
            ...
            raise MATTSyntaxError(f"Unknown command: {token}")
    ...
    def parse_if_statement():
        ...
        if not tokens or tokens.pop(0) != ':':
            raise MATTSyntaxError("Expected ':' after if condition")
        if_body = []
        ...
        if tokens and tokens[0] == f"{COMMAND_PREFIX}ELSE":
            ...
            if not tokens or tokens.pop(0) != ':':
                raise MATTSyntaxError("Expected ':' after else")
            else_body = []
            while tokens and tokens[0] != BLOCK_END:
                else_body.append(parse_statement())
        if not tokens or tokens.pop(0) != BLOCK_END:
            raise MATTSyntaxError(f"Expected '{BLOCK_END}' at the end of if statement")
    ...
    def parse_while_loop():
        ...
        condition = parse_expression()
        if not tokens or tokens.pop(0) != ':':
            raise MATTSyntaxError("Expected ':' after while condition")
        body = []
        while tokens and tokens[0] != BLOCK_END:
            body.append(parse_statement())
        if not tokens or tokens.pop(0) != BLOCK_END:
            raise MATTSyntaxError(f"Expected '{BLOCK_END}' at the end of while loop")
