...
class ALEXSyntaxError(Exception):
    pass

# Hardcoded configuration
LANGUAGE_NAME = "ALEX"
FILE_EXTENSION = ".kleissner"
COMMAND_PREFIX = "ALEX"
COMMENT_PREFIX = "ALEXCOMMENT"
ENFORCE_UPPERCASE_COMMENTS = True
ENFORCE_UPPERCASE_CODE = True
TYPE_PREFIX_INTEGER = "ALEXINT"
TYPE_PREFIX_FLOAT = "ALEXFLOAT"
TYPE_PREFIX_STRING = "ALEXSTRING"
TRUE_VALUE = "TRUTH"
FALSE_VALUE = "LIES"
BLOCK_END = "ALEXEND"
    ...
    for line_number, line in enumerate(lines, 1):
        if COMMENT_PREFIX in line:
            ...
            if ENFORCE_UPPERCASE_COMMENTS and any(c.islower() for c in line[comment_start:]):
                raise ALEXSyntaxError(f"Syntax Error on line {line_number}: Comments must be in all caps")
...
def parse(tokens):
    ...
    def parse_factor():
        if not tokens:
            raise ALEXSyntaxError("Unexpected end of input")
        if tokens[0] == '(':
            tokens.pop(0)  # Remove '('
            result = parse_expression()
            if not tokens or tokens.pop(0) != ')':
                raise ALEXSyntaxError("Expected closing parenthesis")
            return result
            ...
            try:
                ...
                raise ALEXSyntaxError(f"Unexpected token: {tokens[0]}")
        ...
        if tokens and tokens[0] == '(':
            ...
            while tokens and tokens[0] != ')':
                ...
                if tokens and tokens[0] == ',':
                    tokens.pop(0)
            if not tokens or tokens.pop(0) != ')':
                raise ALEXSyntaxError(f"Expected closing parenthesis in function call to {name}")
    ...
    def parse_command():
        ...
        if token in [TYPE_PREFIX_INTEGER, TYPE_PREFIX_FLOAT, TYPE_PREFIX_STRING]:
            if not tokens:
                raise ALEXSyntaxError(f"Expected value after {token}")
            value = tokens.pop(0)
            ...
            raise ALEXSyntaxError(f"Unknown command: {token}")
    ...
    def parse_if_statement():
        ...
        if not tokens or tokens.pop(0) != ':':
            raise ALEXSyntaxError("Expected ':' after if condition")
        if_body = []
        ...
        if tokens and tokens[0] == f"{COMMAND_PREFIX}ELSE":
            ...
            if not tokens or tokens.pop(0) != ':':
                raise ALEXSyntaxError("Expected ':' after else")
            else_body = []
            while tokens and tokens[0] != BLOCK_END:
                else_body.append(parse_statement())
        if not tokens or tokens.pop(0) != BLOCK_END:
            raise ALEXSyntaxError(f"Expected '{BLOCK_END}' at the end of if statement")
    ...
    def parse_while_loop():
        ...
        condition = parse_expression()
        if not tokens or tokens.pop(0) != ':':
            raise ALEXSyntaxError("Expected ':' after while condition")
        body = []
        while tokens and tokens[0] != BLOCK_END:
            body.append(parse_statement())
        if not tokens or tokens.pop(0) != BLOCK_END:
            raise ALEXSyntaxError(f"Expected '{BLOCK_END}' at the end of while loop")
